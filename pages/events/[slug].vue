				...
				<div class="relative z-10 max-w-3xl mx-auto">
					...
					<div class="flex flex-wrap justify-center gap-2.5 mt-8">
						<span class="meta-pill"
							><Calendar class="w-4 h-4" />{{ event.dateTitle }}</span
						>
					...
					<div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
						<div v-for="card in infoCards" :key="card.title" class="info-card">
							<div class="flex items-center gap-3 mb-4">
								...
								<p class="font-display font-bold text-[#1C1530] text-lg">
									{{ card.title }}
								</p>
							</div>
							<div class="grid gap-3">
								<div
									...
									:key="item"
									class="flex gap-2.5 text-sm text-[#4A4360]"
								>
...
.ticket-stub__tail {
	...
	background: repeating-linear-gradient(135deg, #7c5cfc, #7c5cfc 6px, #6b4ce0 6px, #6b4ce0 12px);
}
